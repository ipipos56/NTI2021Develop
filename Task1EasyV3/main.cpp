...
int main()
{
    ...
    for(int co = 0;co<100;co++)
    {
        ...
        if(abs((l1 * cos(beta) - l2 * cos(beta + gamma)) - x0) < 0.01 && abs((l1 * sin(beta) - l2 * sin(beta + gamma)) - y0) < 0.01)
        {
            if(beta_min < beta && beta_max > beta && gamma_min < gamma && gamma_max > gamma)
            {
        ...
        else if(abs((l1 * cos(beta2) - l2 * cos(beta2 + gamma2)) - x0) < 0.01 && abs((l1 * sin(beta2) - l2 * sin(beta2 + gamma2)) - y0) < 0.01)
        {
            if(beta_min < beta2 && beta_max > beta2 && gamma_min < gamma2 && gamma_max > gamma2)
            {
                cout<<beta2<<" "<<gamma2<<endl<<endl;
                continue;
            }
        }
        else if(abs((l1 * cos(beta) - l2 * cos(beta + gamma2)) - x0) < 0.01 && abs((l1 * sin(beta) - l2 * sin(beta + gamma2)) - y0) < 0.01)
        {
            if(beta_min < beta && beta_max > beta && gamma_min < gamma2 && gamma_max > gamma2)
            {
                cout<<beta<<" "<<gamma2<<endl<<endl;
                continue;
            }
        }
        else if(abs((l1 * cos(beta2) - l2 * cos(beta2 + gamma)) - x0) < 0.01 && abs((l1 * sin(beta2) - l2 * sin(beta2 + gamma)) - y0) < 0.01)
        {
            if(beta_min < beta2 && beta_max > beta2 && gamma_min < gamma && gamma_max > gamma)
            {
